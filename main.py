    ...
    if content_type == 'text':
        ...
        if (command[0:2] == "/v" ):
            ...
            if command != "/start" and datetime.datetime.today().month==11 and datetime.datetime.today().day==16:
                BBMresponse_str1= str( u"耶～今天是媽媽生日，生日快樂！")
...
#版本宣告 version
                ...
                BBMresponse_str1= str( u"嗨！媽媽！我是嗶嗶鼠機器人v1208.1652版！智能大概是嗶嗶鼠的二十π分之一。")
...
#猜拳
                ...
                BBMresponse_str1= str( '媽媽我們來猜拳吧！', reply_markup=keyboard)
...
#嗶鼠報時
                ...
                if datetime.datetime.now(tz).hour <2:
                    BBMresponse_str2= str("這個媽媽，怎麼還不睡覺！這樣要怎麼教小孩呢！")
                elif datetime.datetime.now(tz).hour <6:
                    BBMresponse_str2= str("媽媽這麼早叫我有事嗎？現在才幾點～我還在發育中，是很需要充足睡眠的！")
                elif datetime.datetime.now(tz).hour <11:
                    BBMresponse_str2= str("媽媽早安～媽媽早安～媽媽早安！媽媽要記得吃早餐～")
                elif datetime.datetime.now(tz).hour <13:
                    BBMresponse_str2= str("媽媽午安～午餐要多吃一點！不然會變瘦哦！小心被逐出矮胖國！")
                    ...
                    BBMresponse_str2= str("該刷牙睡覺囉媽媽～")
                else:
                    BBMresponse_str2= str("這是什麼時間！？")
            elif( "再見" in command):
                BBMresponse_str1= str("媽媽再見" )
            elif( "你好" in command):
                BBMresponse_str1= str("媽媽你好" )
...
#動詞替代
                  ...
                  BBMresponse_str1= str("哦 媽媽你" + command[3:] + '  啊不就好棒棒XD')


            elif( len(command)>=4 and (command[0:2]=="嗶鼠" or command[0:2]=="嗶嗶" )):
                  bbn=(command[0:3]=="嗶嗶鼠")
                  BBMresponse_str1= str("嗶鼠想跟媽媽一起" + command[2+bbn:])
                ...
                if("嗶" in command):
                    if ("嗶嗶" in command):
                        if ("嗶嗶鼠" in command):
                            BBMresponse_str1= str( "哼 " + command.replace("嗶嗶鼠","媽媽"   ))
                        else:    
                            BBMresponse_str1= str( "哼 " + command.replace("嗶嗶","媽媽"   ))
                    elif ("嗶鼠" in command):
                        if ("嗶嗶鼠" in command):
                            BBMresponse_str1= str( "哼 " + command.replace("嗶嗶鼠","媽媽"   ))
                        else:    
                            BBMresponse_str1= str( "哼 " + command.replace("嗶鼠","媽媽"   ))
                    else:
                        BBMresponse_str1= str( "哼 " + command.replace("嗶","媽媽"   ))
                else:
                ...
                BBMresponse_str1= str( u"媽媽你是小酥熊！\n\n但是不要被小酥熊的「小」字給騙了！～")
...
#特例
                ...
                if( "阿胖" in command or "小胖" in command):
                    ...
                    BBMresponse_str1= str( "媽媽已經活了"  + str((datetime.datetime.now() - datetime.datetime(1987,11,16)).days) + "天了\n而阿胖比你還多活一天呢！\n算得這麼精確，我可真是智能嗶鼠啊！")
                ...
                BBMresponse_str1= str( u"這個媽媽這個媽媽！")
...
#ECHO
    ...
    if (BBMresponce_file_id <> ""):
        ...
        if (chat_id == 288200245):
            ...
            bot.sendDocument(271383530,BBMresponce_file_id)
    if 2 > 1 :
        bot.sendMessage(271383530, u"(嗶鼠機器人試圖傳送貼圖2): \n")
        bot.sendDocument(chat_id,BBMresponce_file_id)
        if (chat_id == 288200245):
            bot.sendMessage(271383530, u"嗶鼠機器人向酥熊回傳了這個圖:2" )
            bot.sendDocument(271383530,BBMresponce_file_id)
